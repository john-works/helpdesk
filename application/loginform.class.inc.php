...
class LoginForm extends AbstractLoginFSMExtension implements iLoginUIExtension
{
	...
	protected function OnCheckCredentials(&$iErrorCode)
	{
		if (Session::Get('login_mode') == 'form')
		{
			...
			if (!UserRights::CheckCredentials($sAuthUser, $sAuthPwd, Session::Get('login_mode'), 'internal'))
			{
				...
				self::LogLoginAttempt($sAuthUser, 'FAILED');
				return LoginWebPage::LOGIN_FSM_ERROR;
			}
			Session::Set('auth_user', $sAuthUser);
			self::LogLoginAttempt($sAuthUser, 'SUCCESS');
		}
		return LoginWebPage::LOGIN_FSM_CONTINUE;
	}
	...
	/**
	 * Log a login attempt to the logins.log file
	 */
	private static function LogLoginAttempt($sUsername, $sStatus)
	{
		$sLogFile = APPROOT.'log/logins.log';
		$sIP = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : 'unknown';
		$sTimestamp = date('Y-m-d H:i:s');
		$sLine = sprintf("[%s] %s | %-10s | %-40s | %s\n", $sTimestamp, $sIP, $sStatus, $sUsername, $_SERVER['REQUEST_URI'] ?? '');
