...
class ActionEmail extends ActionNotification
{
	...
	protected function _DoExecute($oTrigger, $aContextArgs, &$oLog)
	{
		...
		// Auto-prepend ticket ref to subject for threading
		if (isset($aContextArgs["this->object()"]) && $aContextArgs["this->object()"]->IsNew() == false) {
			$oObj = $aContextArgs["this->object()"];
			if ($oObj instanceof Ticket && method_exists($oObj, "GetRef")) {
				$sRef = $oObj->Get("ref");
				if (!empty($sRef) && strpos($aEmailContent["subject"], $sRef) === false) {
					$aEmailContent["subject"] = "[" . $sRef . "] " . $aEmailContent["subject"];
				}
			}
		}
